...
export const filteredSwaps = ([swapsRequest, visibleSwapTokens, swapTypeFilter] : [DataRequest<SwapList>, { value: boolean, name: string }[], { twoAssets: boolean, multipleAssets: boolean }]) => {
...
return (swaps?.swaps || [])
  .filter(
    (swap: Swap) =>
      (swapTypeFilter.twoAssets &&
        Object.keys(swap.tokens).length <= 2) ||
      (swapTypeFilter.multipleAssets &&
        Object.keys(swap.tokens).length > 2)
  )
  .filter((swap: Swap) =>
