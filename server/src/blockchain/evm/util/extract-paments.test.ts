...
describe("extractPayments", () => {
  ...
  test("should return structured token transfers from EVM transfers and transactions", () => {
    ...
    const result: TokenTransfers = extractPayments(transactions, transfers, walletAddress, nativeToken);

    expect(mergeTransfersOfSameTx).toHaveBeenCalledWith(transfers, walletAddress);
    expect(isSwap).toHaveBeenCalled();
    expect(processFunctionName).toHaveBeenCalledWith("transfer(address,uint256)");
  ...
  test("should add native transfers when no matching transfer exists", () => {
    ...
    const result = extractPayments(transactions, transfers, walletAddress, nativeToken);
