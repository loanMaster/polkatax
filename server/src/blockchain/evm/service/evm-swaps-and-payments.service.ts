...
export class EvmSwapsAndPaymentsService {
    ...
    async fetchSwapsAndPayments(network = 'moonbeam', address: string, startDate: Date, endDate: Date): Promise<{ swaps: Swap[], payments: TokenTransfers }> {
        ...
        const {tx, transfers} = await this.evmTxService.fetchTxAndTransfers(network, address, startDate, endDate)
