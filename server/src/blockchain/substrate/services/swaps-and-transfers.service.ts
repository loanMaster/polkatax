...
export class SwapsAndTransfersService {
    ...
    constructor(private blockTimeService: BlockTimeService, 
    ...
    private async fetchTxAndTransfers(chainName: string, address: string, blockMin: number, blockMax: number, evm = false): Promise<{ transactions: Transaction[], transfers: Transfers }> {
    ...
    private async resolveAddresses(chainName: string, address: string) {
        ...
        const substrateAddress = isEvm ? await this.subscanService.mapToSubstrateAccount(chainName, address) : address
    ...
    private async loadAllTransfers(
        ...
        let { transactions, transfers } = substrateAddress
            ? await this.fetchTxAndTransfers(chainName, substrateAddress, blockMin, blockMax, false)
            : { transactions: [], transfers: {} }
    
        if (evmAddress) {
            const evmData = await this.fetchTxAndTransfers(chainName, evmAddress, blockMin, blockMax, true)
    ...
    private filterByDate<T extends { date: number }>(items: T[] | Record<string, T[]>, minDate: Date, maxDate?: Date) {
    ...
    async fetchSwapsAndTransfers(
        ...
        const { substrateAddress, evmAddress } = await this.resolveAddresses(chainName, address)
        const { blockMin, blockMax } = await this.blockTimeService.getMinMaxBlock(chainName, minDate.getTime(), maxDate?.getTime())
    
        let { transactions, transfers } = await this.loadAllTransfers(chainName, substrateAddress, evmAddress, blockMin, blockMax)
        ...
        let payments = this.transferClassifier.extractPayments(transactions, transfers)
        payments = this.filterEmptyTokens(this.filterByDate(payments, minDate, maxDate) as TokenTransfers)
