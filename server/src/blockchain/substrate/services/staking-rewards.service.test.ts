...
jest.mock('../../../common/logger/logger', () => ({ logger: { info: jest.fn() } }))
...
describe('StakingRewardsService', () => {
  ...
  describe('fetchStakingRewards', () => {
    test('should fetch and filter rewards properly', async () => {
      blockTimeService.getMinMaxBlock.mockResolvedValue({ blockMin: 100, blockMax: 200 })
      subscanService.fetchAllStakingRewards.mockResolvedValue([
        { event_id: 'Reward', block_timestamp: 1700000000, block_num: 150, amount: new BigNumber('1000000000000'), hash: 'abc' }
      ])
      subscanService.fetchNativeToken.mockResolvedValue({ token_decimals: 12 } as Token)

      const result = await service.fetchStakingRewards('polkadot', 'address1', 1699999999000)
    ...
    test('should throw error on Slash event', async () => {
      blockTimeService.getMinMaxBlock.mockResolvedValue({ blockMin: 100, blockMax: 200 })
      subscanService.fetchAllStakingRewards.mockResolvedValue([
        { event_id: 'Slash', block_timestamp: 1700000000, block_num: 150, amount: new BigNumber('1000000000000'), hash: 'abc' }
      ])
      subscanService.fetchNativeToken.mockResolvedValue({ token_decimals: 12 } as Token)
  ...
  describe('fetchNominationPoolRewards', () => {
    test('should fetch and filter pool rewards', async () => {
      subscanService.fetchAllPoolStakingRewards.mockResolvedValue([
        { event_id: 'Reward', block_timestamp: 1700000001, block_num: 160, amount: new BigNumber('500000000000'), hash: 'def' }
      ])
      subscanService.fetchNativeToken.mockResolvedValue({ token_decimals: 12 } as Token)

      const result = await service.fetchNominationPoolRewards('moonbeam', 'addr3', 5, 1699999999000)
