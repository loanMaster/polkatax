...
const blocks = []
const fistBlockStartTimestamp = Math.round(new Date('2024-01-01T00:00:00').getTime() / 1000)
...
test('should find block', async () => {
    ...
    const { blockMin, blockMax } = await blockTimeService.getMinMaxBlock('polkadot', minDate, maxDate)
    const minBlock = blocks[blocks.length - blockMin]
    const maxBlock = blocks[blocks.length - blockMax]
    expect(Math.abs((minBlock.block_timestamp - minDate / 1000))).toBeLessThanOrEqual(3 * 3 * 24 * 60 * 60 + 600)
    expect(Math.abs((maxBlock.block_timestamp - maxDate / 1000))).toBeLessThanOrEqual(3 * 3 * 24 * 60 * 60 + 600)
...
test('should find block no latest date', async () => {
    blockTimeService = new BlockTimeService(subscanApi as any);
    const minDate = new Date('2024-06-01T00:00:00').getTime()
    const { blockMax } = await blockTimeService.getMinMaxBlock('polkadot', minDate)
    const maxBlock = blocks[blocks.length - blockMax]
    expect(Math.abs((maxBlock.block_timestamp - Date.now() / 1000))).toBeLessThanOrEqual(3 * 3 * 24 * 60 * 60 + 600)
})
...
test('should return first block if start date lies very far in past', async () => {
    ...
    const { blockMin } = await blockTimeService.getMinMaxBlock('polkadot', minDate)
