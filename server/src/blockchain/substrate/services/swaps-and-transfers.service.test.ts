...
describe('SwapsAndTransfersService', () => {
  ...
  test('should fetch swaps and payments and apply filtering', async () => {
    const fakeTxs = [{ hash: '0x1', block_timestamp: 1700000000, block_num: 100 }]
    const fakeTransfers = { '0x1': { DOT: { amount: 1, timestamp: 1700000000, block: 100, from: 'A', to: 'B', functionName: 'transfer' } } }

    const swaps = [{ hash: '0x1', date: 1700000000 }] as any
    const payments = {
      dot: [{ date: 1700000000, amount: 1, hash: '0x1', from: 'A', to: 'B', block: 100, functionName: 'transfer' }]
    }

    mockSubscanService.mapToSubstrateAccount.mockResolvedValue('substrate-addr')
    mockBlockTimeService.getMinMaxBlock.mockResolvedValue({ blockMin: 10, blockMax: 999 })
    mockSubscanService.fetchAllTx.mockResolvedValue(fakeTxs as Transaction[])
    mockSubscanService.fetchAllTransfers.mockResolvedValue(fakeTransfers as unknown as Transfers)
    mockTransferClassifier.extractSwaps.mockReturnValue(swaps)
    mockTransferClassifier.extractPayments.mockReturnValue(payments)

    const result = await service.fetchSwapsAndTransfers('polkadot', '0xabc', new Date('2023-11-10T00:00:00Z'))

    expect(mockSubscanService.mapToSubstrateAccount).toHaveBeenCalledWith('polkadot', '0xabc')
  ...
  test('should call hydration adjustment only for hydration chain', async () => {
    mockSubscanService.mapToSubstrateAccount.mockResolvedValue('substrate-addr')
    mockBlockTimeService.getMinMaxBlock.mockResolvedValue({ blockMin: 10, blockMax: 999 })
    mockSubscanService.fetchAllTx.mockResolvedValue([{ hash: 'x', block_timestamp: 1700000000, block_num: 99 }] as Transaction[])
    mockSubscanService.fetchAllTransfers.mockResolvedValue({})
    mockTransferClassifier.extractSwaps.mockReturnValue([{ hash: 'x', date: 1700000000 }] as Swap[])
    mockTransferClassifier.extractPayments.mockReturnValue({})

    await service.fetchSwapsAndTransfers('hydration', '0xabc', new Date('2023-11-10'))
  ...
  test('should return empty arrays when no data available', async () => {
    ...
    const result = await service.fetchSwapsAndTransfers('kusama', 'substrate-addr', new Date('2023-01-01'))
