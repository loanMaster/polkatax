...
describe('TransferClassifier', () => {
  ...
  describe('extractPayments', () => {
    test('should extract payments when transfers are one-directional', () => {
      ...
      const transactions: Transaction[] = [
        { hash: 'tx1', functionName: 'transfer', block_num: 1, block_timestamp: 123456, callModule: 'balances' }
      ] as Transaction[]
    ...
    test('should skip if both sent and received exist (swap)', () => {
      const transfers: Transfers = {
        'tx1': {
          'DOT': { from: 'alice', to: 'bob', amount: 100, block: 1, timestamp: 123, functionName: 'transfer' },
          'KSM': { from: 'bob', to: 'alice', amount: -50, block: 1, timestamp: 123, functionName: 'transfer' }
        }
      } as unknown as Transfers
  ...
  describe('extractSwaps', () => {
    test('should extract swaps when transfers are bi-directional', () => {
      const transfers: Transfers = {
        'tx1': {
          'DOT': { from: 'alice', to: 'dex', amount: -100, block: 2, timestamp: 1111, functionName: 'swap' },
          'KSM': { from: 'dex', to: 'alice', amount: 50, block: 2, timestamp: 1111, functionName: 'swap' }
        }
      } as unknown as Transfers

      const transactions: Transaction[] = [
        { hash: 'tx1', functionName: 'swap', block_num: 2, block_timestamp: 1111, callModule: 'dex' }
      ] as Transaction[]
    ...
    test('should fill in block/date from transfer if transaction missing', () => {
      const transfers: Transfers = {
        'tx1': {
          'DOT': { from: 'alice', to: 'bob', amount: -10, block: 5, timestamp: 9999, functionName: 'swap' },
          'KSM': { from: 'bob', to: 'alice', amount: 10, block: 5, timestamp: 9999, functionName: 'swap' }
        }
      } as unknown as Transfers
