...
describe('TokenPriceService', () => {
    ...
    test('fetches prices from API when cache is empty', async () => {
        (tokenUtil.findCoingeckoToken as jest.Mock).mockImplementation((symbol: string, _chain: string) => ({
        ...
        const prices = await service.fetchCurrentPrices(['ETH', 'BTC'], 'ethereum', 'USD');

        expect(mockCoingeckoRestService.fetchPrices).toHaveBeenCalledWith(['eth-id', 'btc-id'], 'usd');
