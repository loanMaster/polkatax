...
test("should mapToSubstrateAccount", async () => {
  const { requestHelper, requestHelperSpy } = setUpRequestHelper({
    data: { account: { substrate_account: { address: "my-address" } } },
  });
  (subscanApi as any)["requestHelper"] = requestHelper;
  const mappedAccount = await subscanApi.mapToSubstrateAccount(
    "polkadot",
    "foo",
  );
  expect(mappedAccount).toBe("my-address");
  expect(requestHelperSpy).toHaveBeenCalledWith(
...
test("should fetch metadata avg block time", async () => {
  let { requestHelper, requestHelperSpy } = setUpRequestHelper({
    data: { avgBlockTime: 6, blockNum: "10" },
  });
  (subscanApi as any)["requestHelper"] = requestHelper;
...
test("should fetchNativeToken", async () => {
  ...
  expect(requestHelperSpy).toHaveBeenCalledWith(
    `https://somechain.api.subscan.io/api/scan/token`,
    "post",
    {},
  );
});
...
test("should fetchBlockList", async () => {
  ...
  expect(requestHelperSpy).toHaveBeenCalledWith(
    `https://kusama.api.subscan.io/api/v2/scan/blocks`,
    "post",
    {
      page: 1,
      row: 100,
...
describe("staking rewards", () => {
  ...
  test("should fetchPoolStakingRewards", async () => {
    ...
    (subscanApi as any)["requestHelper"] = requestHelper;
    const poolStakingRewards = await subscanApi.fetchPoolStakingRewards(
      "kusama",
      "bla",
      1,
      100,
    ...
    expect(requestHelperSpy).toHaveBeenCalledWith(
      `https://kusama.api.subscan.io/api/scan/nomination_pool/rewards`,
      "post",
      {
        row: 100,
        page: 0,
    ...
    const poolStakingRewards2 = await subscanApi.fetchPoolStakingRewards(
      "kusama",
      "bla",
      1,
      2,
  ...
  test("should fetchStakingRewards", async () => {
    ...
    expect(requestHelperSpy).toHaveBeenCalledWith(
      `https://kusama.api.subscan.io/api/scan/account/reward_slash`,
      "post",
      {
        row: 100,
        page: 1,
...
test("should fetchExtrinsics", async () => {
  ...
  expect(requestHelperSpy).toHaveBeenCalledWith(
    `https://polkadot.api.subscan.io/api/v2/scan/extrinsics`,
    "post",
    {
      row: 100,
      page: 0,
