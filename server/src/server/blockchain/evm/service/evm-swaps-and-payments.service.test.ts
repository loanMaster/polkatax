...
describe("EvmSwapsAndPaymentsService", () => {
  ...
  const fakeTx = [
    { hash: "0xabc", timeStamp: `${Math.floor(Date.now() / 1000)}` },
  ];
  ...
  test("should fetch swaps and payments correctly", async () => {
    ...
    const result = await service.fetchSwapsAndPayments(
      "moonbeam",
      "0xTestAddress",
      startDate,
      endDate,
    ...
    expect(mockTxService.fetchTxAndTransfers).toHaveBeenCalledWith(
      "moonbeam",
      "0xTestAddress",
      startDate,
      endDate,
    );
    expect(extractSwaps).toHaveBeenCalledWith(
      fakeTx,
      fakeTransfers,
      "0xtestaddress",
      "GLMR",
    );
    expect(extractPayments).toHaveBeenCalledWith(
      fakeTx,
      fakeTransfers,
      "0xtestaddress",
      "GLMR",
  ...
  test("should lowercase the address before using it", async () => {
    await service.fetchSwapsAndPayments(
      "moonbeam",
      "0xABCDEF",
      new Date(),
      new Date(),
  ...
  test("should log entry and exit", async () => {
    ...
    expect(logger.info).toHaveBeenCalledWith(
      "Enter fetchSwapsAndPayments for moonbeam",
    );
    expect(logger.info).toHaveBeenCalledWith(
      "Exit fetchSwapsAndPayments for moonbeam",
    );
  });
});
