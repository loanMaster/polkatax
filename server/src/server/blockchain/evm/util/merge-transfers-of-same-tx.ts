...
export const mergeTransfersOfSameTx = (
  ...
  transfers.forEach((t) => {
    ...
    const value = new BigNumber(t.value)
      .dividedBy(new BigNumber(10).exponentiatedBy(Number(t.tokenDecimal)))
      .toNumber();
    const valueUpdate =
      t.from === walletAddress ? -value : t.to === walletAddress ? value : 0;
    ...
    transfersObj[t.hash].tokens[tokenSymbol] =
      transfersObj[t.hash].tokens[tokenSymbol] === undefined
        ? valueUpdate
        : transfersObj[t.hash].tokens[tokenSymbol] + valueUpdate;
  });
  return transfersObj;
};
