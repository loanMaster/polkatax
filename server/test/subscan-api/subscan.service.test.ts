...
import {SubscanApi} from "../../src/subscan-api/subscan.api";
...
test('should fetchAllStakingRewards', async () => {
    const subscanApi = {
        fetchStakingRewards: async (chainName: string, address: string, row: number = 100, page: number = 0, isStash: boolean, block_min?: number, block_max?: number) => {
            if (page === 0) {
                return {
                    list: [{
                        event_id: "Reward",
    ...
    const stakingRewards = await subscanService.fetchAllStakingRewards("polkadot", "alice", 100, 200)
...
test('should fetchAllPoolStakingRewards', async () => {
    const subscanApi = {
        fetchPoolStakingRewards: async (chainName, address, poolId, count, page) => {
            if (page === 0) {
                return {
                    list: ['a', 'b'], hasNext: true
                }
            } else {
                return {
                    list: ['c'], hasNext: false
                }
            }
        }
    }
    ...
    subscanService = new SubscanService(subscanApi as any);
    const stakingRewards = await subscanService.fetchAllPoolStakingRewards("polkadot", "alice", 2)
...
test('should fetchAllTx', async () => {
    const subscanApi = {
        fetchExtrinsics: async (chainName: string, address: string, row: number = 100, page: number = 0, block_min?: number, block_max?: number, evm = false) => {
            if (page === 0) {
                return {
                    list: ['a', 'd'], hasNext: true
                }
            } else {
                return {
                    list: ['k'], hasNext: false
                }
            }
        }
    }
    ...
    subscanService = new SubscanService(subscanApi as any);
    const tx = await subscanService.fetchAllTx("polkadot", "alice", 10, 20, false)
...
test('should fetchAllTransfers', async () => {
    const subscanApi = {
        ...
        fetchTransfers: async (chainName: string, account: string, isMyAccount: (string) => boolean, row: number = 100, page: number = 0, block_min?: number, block_max?: number, evm = false) => {
            ...
            if (page === 0) {
                return {
                    list: [{ a: 0, b: 1 }], hasNext: true
                }
            } else {
                return {
                    list: [{ c: 3 }], hasNext: false
                }
            }
        }
    }
    subscanService = new SubscanService(subscanApi as any);
    const tx = await subscanService.fetchAllTransfers("polkadot", "Ally", 10, 20, false)
    expect(tx).toEqual({
        a: 0,
        b: 1,
        c: 3
